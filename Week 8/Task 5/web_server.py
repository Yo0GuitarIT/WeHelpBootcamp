...
def sign_up():
    if request.method == "POST":
        ...
        if not (validate(name) and validate(username) and validate(password)):
...
def sign_in():
    ...
    try:
        ...
        connection.close()
        print("Error:", ex)
        return redirect("/error?message=An error occurred during login.")
...
def create_Message():
    if "member_id" in session:
        ...
        if not validate(message_content):
        ...
        try:
            ...
            with connection.cursor(dictionary=True) as cursor:
                ...
                cursor.execute(sql_insert, (member_id, message_content))
                connection.commit()
        ...
        if request.method == "GET":
            try:
                ...
                if not validate(username):
                    return jsonify({"error": True, "message": "Invalid input"}), 400
